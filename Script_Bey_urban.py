# -*- coding: utf-8 -*-
"""
Simulation LR-FHSS - Modele radio Beyrouth Urbain
...
Distributions :
  ...
  - h4=0.50 / h5=0.50
  - h2=h3=h4=h5=0.25
Distances : 1,2,3,4,5,6,8,10,12,14,16 km
Moyenne sur 10 seeds (0-9), parallelisation joblib
"""
...
# ============================================================
# Modele radio Beyrouth Urbain (El Chall et al., 2019)
# ============================================================
N_EXP    = 4.18
PL0      = 102.86
LH       = -6.3
...
# ============================================================
def job(label, groups_spec, n_nodes, distance_km, seed):
    rate = run_sim_groups(
        groups_spec  = groups_spec,
        total_nodes  = n_nodes // 8,
        distance_km  = distance_km,
        seed         = seed,
    )
    return label, n_nodes, distance_km, rate
...
if __name__ == "__main__":
    ...
    DR              = 'DR8'
    N_NODES         = 150000 // 8   # noeuds simules
    BASE            = 'acrda'
    CODE            = '1/3'
    SEEDS           = list(range(10))
    DISTANCES       = [1, 2, 3, 4, 5, 6, 8, 10, 12, 14, 16]
    ...
    # Info couverture
    print("=== Modele : Beyrouth Urbain (El Chall et al., 2019) ===")
    print(f"  P_tx={P_TX_DBM} dBm | n={N_EXP} | PL0={PL0} | h_ED={H_ED} m")
    print()
    ...
    # Construction et execution des jobs
    jobs = [
        (label, groups_spec, N_NODES * 8, d, s)
    ...
    results = Parallel(n_jobs=-1, verbose=0)(
        delayed(job)(label, groups_spec, n, d, s)
        for label, groups_spec, n, d, s in jobs
    )

    # Agregation par (label, distance)
    bucket = defaultdict(list)
    for label, n, d, rate in results:
        if rate is not None:
            bucket[(label, d)].append(rate)

    # Affichage par distribution
    for label, groups_spec in DISTRIBUTIONS:
        print(f"=== {label} ===")
