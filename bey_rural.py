# -*- coding: utf-8 -*-
"""
Created on Tue Apr  7 14:57:55 2026

@author: tanios boudiwan
"""

# -*- coding: utf-8 -*-
"""
Simulation LR-FHSS avec modèle radio Beyrouth Rural (Bekaa Valley)
...
# ============================================================
# Modèle radio Beyrouth Rural - Bekaa Valley (El Chall et al., 2019)
# PL = 10n × log10(d) + PL0 + Lh × log10(hED)
# ============================================================
N_EXP    = 3.033     # exposant path loss rural Bekaa
PL0      = 111.75    # path loss de référence
LH       = -6.65     # facteur hauteur antenne end-device
...
if __name__ == "__main__":
    ...
    #distances = [1,6,8,10,12,14,16,18,20,22]
    distances = [1,10,12,14,16,18,20,22]
    ...
    print("=== Modèle : Beyrouth Rural - Bekaa Valley (El Chall et al., 2019) ===")
    print(f"  Fréquence  : {F_MHZ} MHz")
    print(f"  P_tx       : {P_TX_DBM} dBm")
    print(f"  n (exposant): {N_EXP}  ← plus faible qu'urbain (3.033 vs 4.18)")
    print(f"  PL0        : {PL0} dB")
    print(f"  Lh         : {LH} dB")
    ...
    # Comparaison avec Beyrouth urbain
    print("=== Comparaison Rural vs Urbain (portée DR8, h_ED=1.5m) ===")
    print(f"  Rural  (n=3.033, PL0=111.75) : {max_distance('DR8', H_ED_M):.2f} km")
    print(f"  Urbain (n=4.18,  PL0=102.86) : "
          f"{10 ** ((P_TX_DBM - 102.86 - (-6.3)*np.log10(H_ED_M) - SENSITIVITY['DR8']) / (10*4.18)):.2f} km")
    print()
...
'''
    ...
    filename  = f"resultats_beyrouth_rural_{DR}_{N_NODES*8}noeuds_{timestamp}.xlsx"
    ...
    with pd.ExcelWriter(filename, engine='openpyxl') as writer:
        ...
        params_data = {
            'Paramètre': [
                'Modèle radio', 'Environnement', 'Fréquence (MHz)',
                'P_tx (dBm)', 'n (exposant)', 'PL0 (dB)',
                'Lh', 'h_ED (m)', 'DR',
                'Nœuds (sim)', 'Nœuds (réseau)', 'Base',
                'Code rate', 'Seed', 'Portée DR8 (km)',
                'Distributions testées'
            ],
            'Valeur': [
                'Beyrouth Rural (El Chall et al., 2019)',
                'Bekaa Valley (rural, altitude 900m)',
                F_MHZ, P_TX_DBM,
                N_EXP, PL0, LH, H_ED_M, DR,
                N_NODES, N_NODES * 8, BASE,
                CODE, SEED, round(max_distance('DR8', H_ED_M), 2),
